 ...
 *   1. DELETE existing symbols/occurrences for this index (cascade handles it via FK)
 *   2. BULK INSERT symbols in chunks
 *   3. BULK INSERT occurrences in chunks
 *   4. UPDATE indexes.status = 'ready' + updated_at
 *   5. UPSERT repo_head (latest indexed commit per branch)
 */
export async function materialize(
  ...
  // 4. Mark index as ready
  await client.query(
    "UPDATE indexes SET status = 'ready', updated_at = NOW() WHERE id = $1",
    [indexId]
  );

  // 5. Upsert repo_head (only if we have a branch)
  if (branch) {
    await client.query(
